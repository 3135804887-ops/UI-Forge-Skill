...
export function containsMetadataUrl(value) {
  function scan(current, inCodeBlock = false) {
    if (typeof current === "string") return URL_PATTERN.test(current);
    if (Array.isArray(current)) return current.some((entry) => scan(entry, inCodeBlock));
    if (!isObject(current)) return false;
    return Object.entries(current).some(([key, entry]) => {
      if (inCodeBlock && key === "code") return false;
      if (key === "code_blocks" && Array.isArray(entry)) {
        return entry.some((block) => scan(block, true));
      }
      return scan(entry, inCodeBlock);
    });
  }
  return scan(value);
}
