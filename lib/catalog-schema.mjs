...
export const STATUS_CONTRACT = Object.freeze({
  complete: Object.freeze({ confidence: 1, code: "required", unresolved_local_import: "forbidden" }),
  recoverable: Object.freeze({ confidence: 0.85, code: "required", unresolved_local_import: "forbidden" }),
  incomplete: Object.freeze({ confidence: 0.5, code: "required", unresolved_local_import: "required" }),
  invalid: Object.freeze({ confidence: 0, code: "forbidden", unresolved_local_import: "forbidden" }),
});
...
export function deriveReconstructionStatus({ hasUsableCode, hasUnresolvedLocalImport, reconstructionApplied }) {
  if (!hasUsableCode) return "invalid";
  if (hasUnresolvedLocalImport) return "incomplete";
  if (reconstructionApplied) return "recoverable";
  return "complete";
}
...
export function validateRecord(record, file = "<memory>") {
  ...
  } else if (Object.hasOwn(STATUS_CONTRACT, record.status) && record.confidence !== STATUS_CONTRACT[record.status].confidence) {
    errors.push(issue(
      "STATUS_CONFIDENCE_MISMATCH",
      file,
      "confidence",
      `${record.status} status requires confidence ${STATUS_CONTRACT[record.status].confidence}`,
    ));
  }
  ...
  if (!Array.isArray(record.code_blocks) || (record.code_blocks.length === 0 && record.status !== "invalid")) {
    errors.push(issue("MISSING_CODE", file, "code_blocks", "code_blocks must contain at least one block"));
  } else {
    if (record.status === "invalid" && record.code_blocks.length > 0) {
      errors.push(issue("STATUS_CODE_MISMATCH", file, "code_blocks", "invalid status forbids code blocks"));
    }
  ...
  } else if (Object.hasOwn(STATUS_CONTRACT, record.status)) {
    const hasUnresolvedLocalImport = record.diagnostics.some((diagnostic) => diagnostic?.code === "UNRESOLVED_LOCAL_IMPORT");
    const requirement = STATUS_CONTRACT[record.status].unresolved_local_import;
    if ((requirement === "required") !== hasUnresolvedLocalImport) {
      errors.push(issue(
        "STATUS_DIAGNOSTIC_MISMATCH",
        file,
        "diagnostics",
        `${record.status} status ${requirement === "required" ? "requires" : "forbids"} UNRESOLVED_LOCAL_IMPORT diagnostics`,
      ));
    }
  }
