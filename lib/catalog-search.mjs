...
function attachRelatedCategories(results, matches, requestedCategory) {
  const bestByCategory = new Map();
  for (const match of matches) {
    const normalizedCategory = normalize(match.record.category);
    if (normalizedCategory === requestedCategory || bestByCategory.has(match.record.category)) continue;
    bestByCategory.set(match.record.category, match.score);
  }
  const relatedCategories = [...bestByCategory]
    .sort(([leftCategory, leftScore], [rightCategory, rightScore]) => (
      rightScore - leftScore || compareTitles(leftCategory, rightCategory)
    ))
    .map(([category]) => category);
  Object.defineProperty(results, "relatedCategories", {
...
export function searchCatalog(records, query, options = {}) {
  ...
  const results = matches.slice(0, limit).map(toSummary);

  if (results.length === 0 && normalizedCategory !== undefined) {
    const unfilteredMatches = findMatches(records, normalizedQuery, queryTokens, options, undefined);
    attachRelatedCategories(results, unfilteredMatches, normalizedCategory);
  }
  return results;
}
