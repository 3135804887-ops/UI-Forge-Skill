...
import { deriveReconstructionStatus, SCHEMA_VERSION, STATUS_CONTRACT } from "./catalog-schema.mjs";
...
function sanitizeMetadataText(value, fallback) {
  ...
  const sanitized = original
    ...
    .replace(/\s+/g, " ")
    .trim();
  return { value: sanitized || fallback, removed: sanitized !== original.trim() };
}
...
function normalizeLegacyCodeBlocks(value) {
  ...
  value.forEach((block, index) => {
    if (typeof block === "string") {
      code_blocks.push({ index: code_blocks.length, code: normalizeCode(block) });
      return;
    }
...
function normalizedRecord(group, normalizedUrl) {
  ...
  const status = deriveReconstructionStatus({
    hasUsableCode,
    hasUnresolvedLocalImport: analysis.unresolved_local_imports.length > 0,
...
export async function loadLegacyRecords({ sourcePath }) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      const title = sanitizeMetadataText(legacyRecord.title, "Untitled component");
      const description = sanitizeMetadataText(legacyRecord.description, "External source description removed.");
      const category = sanitizeMetadataText(legacyRecord.category, "uncategorized");
      const metadataDiagnostics = [
        ...
        .map(([field]) => ({ code: "METADATA_URL_REMOVED", message: `Removed URL from legacy ${field} metadata.` }));
      ...
      candidates.push({
        ...
        diagnostics: [...metadataDiagnostics, ...normalizedBlocks.diagnostics],
        recovered,
      });
...
export async function buildCatalog({ sourcePath, outputPath } = {}, internalDependencies = {}) {
  ...
  if (await exists(backup)) {
    throw builderError("BUILD_PATH_BUSY", `promotion recovery path already exists: ${backup}`);
  }
  ...
  const catalogValidator = internalDependencies.catalogValidator ?? validateCatalog;
  ...
  try {
    ...
    const validation = await catalogValidator(temporary);
    if (!validation.valid) {
    ...
    return { manifest, report, buildReport, outputPath: output, warnings };
  } catch (error) {
