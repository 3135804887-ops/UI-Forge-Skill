...
function rollbackError(message, cause, rollback) {
  const error = builderError("PROMOTION_ROLLBACK_FAILED", message);
  error.cause = cause;
...
export async function buildCatalog({ sourcePath, outputPath } = {}, internalDependencies = {}) {
  if (!(sourcePath instanceof URL) && (typeof sourcePath !== "string" || sourcePath.trim().length === 0)) {
    throw builderError("INVALID_SOURCE_PATH", "sourcePath must be a nonblank path string or file URL");
  }
  ...
  let source;
  try {
    source = sourcePath instanceof URL ? fileURLToPath(sourcePath) : resolve(sourcePath);
  } catch (error) {
    const invalid = builderError("INVALID_SOURCE_PATH", "sourcePath URL must use the file: protocol");
    invalid.cause = error;
    throw invalid;
  }
  const output = resolve(outputPath);
  const canonicalSource = await realpath(source);
  const canonicalOutput = await canonicalProspectivePath(output);
  ...
  if (isWithin(canonicalOutput, canonicalSource)) {
  ...
  for (const reservedPath of [backup, failedOutput]) {
    if (await exists(reservedPath)) {
      throw builderError("BUILD_PATH_BUSY", `promotion recovery path already exists: ${reservedPath}`);
    }
  }
  ...
  try {
    ...
    let backedUp = false;
    if (hadOutput) {
      await promotionRename(output, backup);
      backedUp = true;
    }
    try {
      await promotionRename(temporary, output);
    } catch (error) {
      if (backedUp) {
        try {
          await promotionRename(backup, output);
        } catch (restoreError) {
          throw rollbackError(
            `catalog promotion failed and the previous output could not be restored; recovery backup remains at ${backup}`,
            error,
            {
              output_path: output,
              backup_path: backup,
              new_output_path: temporary,
              old_output_restored: false,
              new_output_preserved: await exists(temporary),
    ...
    if (backedUp) {
      try {
        await promotionRm(backup, { recursive: true });
      } catch (cleanupError) {
        const rollback = {
          output_path: output,
          backup_path: backup,
          new_output_path: failedOutput,
          old_output_restored: false,
          new_output_preserved: true,
        };
        try {
          await promotionRename(output, failedOutput);
        } catch (moveNewError) {
          rollback.new_output_path = output;
          rollback.move_new_error = moveNewError.message;
          throw rollbackError(
            `backup cleanup failed and the new output could not be moved aside; old output remains at ${backup}`,
            cleanupError,
            rollback,
          );
        }
        try {
          await promotionRename(backup, output);
          rollback.old_output_restored = true;
        } catch (restoreError) {
          rollback.restore_error = restoreError.message;
          throw rollbackError(
            `backup cleanup failed and the previous output could not be restored; old and new outputs remain at the reported recovery paths`,
            cleanupError,
            rollback,
          );
        }
        try {
          await promotionRm(failedOutput, { recursive: true });
          rollback.new_output_preserved = false;
        } catch (removeNewError) {
          rollback.remove_new_error = removeNewError.message;
          throw rollbackError(
            `the previous output was restored, but cleanup of the rolled-back new output failed at ${failedOutput}`,
            cleanupError,
            rollback,
          );
        }
        const error = builderError("BACKUP_CLEANUP_FAILED", "backup cleanup failed; the new output was rolled back and the previous output was restored");
    ...
    if (error?.code !== "PROMOTION_ROLLBACK_FAILED") {
      await rm(temporary, { recursive: true, force: true });
    }
    throw error;
  }
