...
export async function loadCatalog(catalogPath) {
    ...
    files: Array.isArray(manifest.files)
      ? manifest.files.map((entry) => (
        entry !== null && typeof entry === "object" && typeof entry.path === "string"
          ? { ...entry, path: normalizeComponentPath(entry.path) }
          : entry
      ))
      : manifest.files,
  };
  const manifestErrors = validateManifest(normalizedManifest, manifestFile);
  if (manifestErrors.length > 0) {
    const first = manifestErrors[0];
    throw makeError(first.code, first.message, first.file, first.path);
  }

  if (computeManifestDigest(normalizedManifest.files) !== normalizedManifest.digest) {
    throw makeError("MANIFEST_DIGEST_MISMATCH", "Manifest digest does not match its contents", manifestFile, "digest");
  }

  const sortedFiles = [...normalizedManifest.files].sort((left, right) => compareStrings(left.path, right.path));
  const records = [];
  const ids = new Set();

  for (const entry of sortedFiles) {
    const { resolvedPath: file } = await resolveComponentFile(resolvedPath, entry.path);
  ...
  return {
    path: resolvedPath,
    manifest: { ...normalizedManifest, files: sortedFiles },
    records,
    errors: [],
    warnings: [],
  };
