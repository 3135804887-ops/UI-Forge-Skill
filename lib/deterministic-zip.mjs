...
import { canonicalizeProspectivePath, isPathContained, normalizeComponentPath } from "./catalog-integrity.mjs";
...
/**
 * Compare canonical or prospective paths using the host platform's filename
 * case semantics. Existing aliases are already resolved by realpath; this
 * additionally protects non-existing Windows targets from case-only aliases.
 */
...
function packagePathCollision(message) {
  const error = new Error(message);
  error.code = "PACKAGE_PATH_COLLISION";
  return error;
}

const OPTIONAL_CATALOG_FILES = Object.freeze([
  "reports/build-report.json",
  "reports/duplicate-groups.json",
  "reports/rejected-records.json",
]);

function catalogEntryError(code, entry, message) {
  const error = new Error(message);
  error.code = code;
  error.file = entry;
  return error;
...
async function listFiles(root, allowedPaths, operations = {}) {
  ...
  async function walk(directory) {
    for (const entry of await list(directory, { withFileTypes: true })) {
      ...
      let normalizedPath;
      try { normalizedPath = normalizeComponentPath(logicalPath); } catch {
        throw catalogEntryError("INVALID_CATALOG_ENTRY_PATH", logicalPath, `Invalid catalog entry path: ${logicalPath}`);
      }
      const info = await inspect(absolute);
      if (info.isSymbolicLink()) {
        throw catalogEntryError("UNSUPPORTED_CATALOG_ENTRY", normalizedPath, `Catalog entry must not be a symbolic link or junction: ${normalizedPath}`);
      }
      if (info.isDirectory()) {
        if (!allowedPaths.directories.has(normalizedPath)) {
          throw catalogEntryError("UNEXPECTED_CATALOG_ENTRY", normalizedPath, `Unexpected catalog directory: ${normalizedPath}`);
        }
        await walk(absolute);
      }
      else if (info.isFile()) {
        if (!allowedPaths.has(normalizedPath)) {
          throw catalogEntryError("UNEXPECTED_CATALOG_ENTRY", normalizedPath, `Unexpected catalog entry: ${normalizedPath}`);
        }
        files.push({ absolute, path: normalizedPath });
      } else {
        throw catalogEntryError("UNSUPPORTED_CATALOG_ENTRY", normalizedPath, `Catalog entry must be a regular file: ${normalizedPath}`);
      }
    }
  }
  await walk(root);
  const found = new Set(files.map((file) => file.path));
  for (const required of allowedPaths.required) {
    if (!found.has(required)) throw catalogEntryError("MISSING_CATALOG_ENTRY", required, `Missing catalog entry: ${required}`);
  }
...
async function writeDeterministicZip({ source, temporary, allowedPaths }, operations = {}) {
  const read = operations.readFile ?? readFile;
  const files = await listFiles(source, allowedPaths, operations);
...
export async function packageCatalog({ sourcePath, outputPath, checksumPath }, internalDependencies = {}) {
  ...
  const canonicalOutput = await canonicalizeProspectivePath(output);
  const canonicalChecksum = await canonicalizeProspectivePath(checksum);
  if (pathsReferToSameLocation(canonicalOutput, canonicalChecksum)) {
    throw packagePathCollision("checksumPath must be distinct from outputPath");
  }
  for (const [label, physical] of [["outputPath", canonicalOutput], ["checksumPath", canonicalChecksum]]) {
    if (pathsReferToSameLocation(canonicalSource, physical) || isPathContained(canonicalSource, physical)) {
      throw packagePathCollision(`${label} must be outside sourcePath`);
    }
  }
  ...
  const requiredPaths = new Set([
    "manifest.json",
    ...loaded.manifest.files.map((entry) => `components/${normalizeComponentPath(entry.path)}`),
  ]);
  const allowedPaths = new Set([...requiredPaths, ...OPTIONAL_CATALOG_FILES]);
  ...
  Object.defineProperty(allowedPaths, "required", { value: requiredPaths });
  Object.defineProperty(allowedPaths, "directories", { value: allowedDirectories });
  ...
  try {
    summary = await writeDeterministicZip({ source, temporary: zipTemporary, allowedPaths }, internalDependencies.archiveOperations);
