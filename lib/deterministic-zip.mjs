...
export async function packageCatalog({ sourcePath, outputPath, checksumPath }, internalDependencies = {}) {
  ...
  if (canonicalOutput === canonicalChecksum) throw new Error("checksumPath must be distinct from outputPath");
  for (const [label, target] of [["outputPath", output], ["checksumPath", checksum]]) {
    const physical = label === "outputPath" ? canonicalOutput : canonicalChecksum;
    if (isPathContained(canonicalSource, physical)) throw new Error(`${label} must be outside sourcePath`);
  }
